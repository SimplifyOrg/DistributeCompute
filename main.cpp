...
int main(int argc, char** argv){
    ...
    rmqt::ExchangeHandle exch = topology.addExchange("exch-name1");
    rmqt::QueueHandle queue   = topology.addQueue("queue-name1");

    topology.bind(exch, queue, "routingkey1");
    ...
    if (!producerResult) {
        ...
        std::cout << "Producer result created." << std::endl; 
    }
        ...
        [](const rmqt::Message& message,
           ...
           const rmqt::ConfirmResponse& response) {
            ...
            if (response.status() == rmqt::ConfirmResponse::ACK) {
                ...
                std::cout << "Message received by broker." << std::endl; 
            }
            else {
    ...
    std::cout << "Producer message send succefully." << std::endl; 
}
